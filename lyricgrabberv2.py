...
def getartists(letter):
    ...
    #response = urllib2.urlopen("http://www.azlyrics.com/a.html")
...
def getsongs(artist_url):
    ...
    url+=artist_url
    #response = urllib2.urlopen("http://www.azlyrics.com/b/blink.html")
    time.sleep(random.randrange(1,3))
    response = urllib2.urlopen(url)
...
def getlyrics(song_url):
    ...
    #response = urllib2.urlopen("http://www.azlyrics.com/lyrics/katyperry/ifyoucanaffordme.html")
    #look at utils and see how amny 200 or 400 I got
    #response = urllib2.urlopen(url)

    try:
        response = urllib2.urlopen(url)
        #print("everything went well continue")
...
if __name__=="__main__":
    ...
    #get artist and name
    testf = open('topsongslyrictest.txt','w')
    for song in songs:
        artist = song["artist"]
        title = song["title"]
        #song_url = "/lyrics/keithanderson/adaliene.html"
        ...
        lyrics = getlyrics(song_url)
            if(lyrics == "error"):
                errorcount+=1
        testf.write('{"artist": '+artist+', "title": '+title+', "lyrics": '+lyrics+'}\n')
        #testf.write(songurl+'\n')
    
    testf.write('# of errors: '+errorcount+'\n')
    testf.close()
    end_time = time.time()
